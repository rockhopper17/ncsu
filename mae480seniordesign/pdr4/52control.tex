...
\\
The driving factor into choosing a GNC actuator system is what kind of control authority it offers to the system. Other systems but not limited to such as thermal, communication, and payload depend on this controllability for mission success. For the STARS mission, the payload pointing is key to ensure that it is pointed towards the Van Allen belts for observation of the belts. Additional maneuvers may be required dependent on subsystem requirements. Due to this being a qualitative measure versus a quantitative measure, a score of 3 indicates that all maneuvers are capable of being met and values higher and lower than 3 are qualitative measures. The restricted and unrestricted 3DoF control is in reference to the reaction wheels becoming saturated after being used. 
\item{Heritage} \\ 
\\ 
...
In order to have mission success of any spacecraft, proper GNC actuators must be selected for robust control of the system. As GNC actuators provide control the system, the control authority of a system is crucial. Heritage is crucial for a GNC system to validate its use. Mass and Volume are last but equally important due to the constraints of fitting within a 3U CubeSat system as used by STARS. As a side comment, the system possibilities are the following, torque rods independently, reaction wheels with torque rods, or reaction control system, reaction wheels, and torque rods. On the potential Pugh matrix, these combinations are implied but are not written out. 
...
\hline
    ...
    \\
\hline
    \textbf{Total} & 1 & 4.0 & 4.5 &  3.4
    \\
\hline
\end{tabular}
\caption{Preliminar Control Pugh Matrix }
