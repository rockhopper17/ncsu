...
implicit none
	...
	! global variables ========================
	real(8), dimension(nx,ny) :: xn,yn		! x,y grid coordinates
	real(8), dimension(nx,ny) :: u,un		! sln(k),sln(k+1)
	real(8), dimension(nx,ny) :: K			! transmissivity func
	...
	do while (t.le.tstop)
		...
		if (doprint) then	
			...
			if (mod(t,0.1).le.0.0001) then
				write(*,"(5E20.8)") t,Q,u(10,10),u(nx/2,ny/2),u(nx-10,ny-10)
			end if
