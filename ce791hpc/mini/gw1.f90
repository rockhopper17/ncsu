...
include "mpif.h"
	...
	integer i,j,tidx,j0		! reserve i,j for indexing
	...
	! boundary conditions for u result from same equation as interior
	do j = 0,nx+1
	do i = 0,ny+1
		xval = (xbase+j)*dx
		yval = i*dy
	...
	if (doprint) then
		...
		! always print the first ghost row at y=0
		if (iam.eq.0) then
			j0 = 0
		else
			j0 = 1
