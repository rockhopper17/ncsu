...
implicit none
	...
	!real(8), parameter :: tstart = 0.0, tstop = 0.01  ! start,stop times
	!real(8), parameter :: tstart = 0.0, tstop = 1.0  ! start,stop times
	real(8), parameter :: tstart = 0.0, tstop = 5.0  ! start,stop times
	!real(8), parameter :: tstart = 0.0, tstop = 0.5  ! start,stop times
	integer, parameter :: niter = 1E2			! max num iter for elliptic solver
	integer, parameter :: iteropt = 2			! 1=Jacobi, 2=GS/SOR
	real(8), parameter :: omega = 1.0			! relaxation parameter for SOR
	real(8), parameter :: tol = 1E-4			! residual tolerance
...
use grid_data
	...
	do i = 1,nx+1
		...
		!veln = u(i,j)*nhat_i(1,i,j) + v(i,j)*nhat_i(2,i,j) ! vn = uf*nx + vf*ny
	...
	do i = 1,nx
		...
		!veln = u(i,j)*nhat_j(1,i,j) + v(i,j)*nhat_j(2,i,j)
...
!==============================================================================
! calculate kinetic energy and velocity divergence
!==============================================================================
subroutine calc_evolution
use grid_data

	real(8) totdiv,totk

	! update the velocity divergence values
	call ode_dveldt
...
write(*,*) 'tot vel divergence = ', totdiv,'tot kinetic = ',totk

end subroutine calc_evolution
...
!if (i.eq.2.and.j.eq.2) then
	!write(*,*) 'fint = ',fint,' pcur=',pcur,' sigma=',sigma
!end if
			! Jacobi
...
use grid_data
	...
	! print the top left corner of pressure
