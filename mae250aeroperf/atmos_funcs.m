function fh = atmos_funcs
fh.simple_atmos = @simple_atmos;
fh.grad_layer = @grad_layer;
fh.isothermal = @isothermal;
fh.simple_atmos_writetable = @simple_atmos_writetable;
end


% function to create table of values and write out to excel sheet
%    to be used for export to pdf
function [h, hG, T, p, rho, a_inf, mu, tbl] = simple_atmos_writetable(h)

% geopotential h (option = 1)
[h, hG, T, p, rho, a_inf, mu] = simple_atmos(h, 1);

tbl = table(h,T,p,rho,a_inf,mu);
writetable(tbl, 'simple_atmos_table.xlsx', 'Range', 'A1');

% geometric hG (option = 0)
[h, hG, T, p, rho, a_inf, mu] = simple_atmos(h, 0);

tbl = table(hG,T,p,rho,a_inf,mu);
writetable(tbl, 'simple_atmos_table.xlsx', 'Range', 'A15');
