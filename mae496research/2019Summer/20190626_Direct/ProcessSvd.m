...
slides = Presentation('inputsignal');
...
for idxpt = 1:numpts
	...
	figure(idxpt);
	sgtitle(strcat('Direct Bond LDV Point ',num2str(pos)));
	subplot(8,2,fidx);
...
for idxpt = 1:numpts
	...
	% save to powerpoint
	imgname = sprintf('%s_pwelch_pt%d.jpg',replace(svdtitle,"_","\_"),pos);
	saveas(gcf,imgname);
	img = Picture(imgname);
	%img.Width = '13.33in';
	img.Width = '10.5in';
...
% save to powerpoint when creating subplots
for idxpt = 1:numpts
	pos = npos(idxpt);
	figure(idxpt);
	imgname = sprintf('jpg/time_pt%d.jpg',pos);
	saveas(gcf,imgname);
	img = Picture(imgname);
	img.Width = '10.5in';
	img.Height = '7.5in';
