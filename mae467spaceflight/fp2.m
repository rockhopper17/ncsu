...
title({'Comparison of delta-v and flight times for Hohmann and Bi-elliptic','7000km orbit to 140000km orbit'});
set(gca,'FontSize',14);
legend show;
...
% Orbit 1 Attributes
a_orb1 = (rp_orb1+ra_orb1)/2; % semimajor axis
e_orb1 = (ra_orb1-rp_orb1)/(ra_orb1+rp_orb1);
h_orb1 = sqrt(rp_orb1 * mu * (1 + e_orb1));
T_orb1 = (2*pi*a_orb1^(3/2))/sqrt(mu);
b_orb1 = sqrt((a_orb1^2)*(1-e_orb1^2));% semiminor axis
foc1 = sqrt(a_orb1^2 - b_orb1^2);
%
...
% Pre-allocations
Ec_p = zeros(1,length(MeC_p));
rC_p = zeros(3,length(MeC_p)); vC_p1 = zeros(3,length(MeC_p));
vB2 = zeros(3,length(MeC_p)); vC_p2 = zeros(3,length(MeC_p));
deltaV_total = zeros(1,length(MeC_p));orb_Elements =zeros(1,length(MeC_p));
...
for i = 1:length(MeC_p)
rC_p(:,i) = (((h_orb1^2)/mu)*(1/(1+e_orb1*cosd(CpTheta(i)))))*...
    [cosd(CpTheta(i)) sind(CpTheta(i)) 0]';
vC_p1(:,i) = (mu/h_orb1)*[-sind(CpTheta(i)) e_orb1+cosd(CpTheta(i)) 0]';

% solving Lambert's Problem using book
[vB2(:,i),vC_p2(:,i)] = lambert_eq(rB1,rC_p(:,i),delta_timRang(i),'pro');
end

% solve the delta _v required for spacecarft B
deltaVb = vB2 - vB1;
deltaVc_p = vC_p1 - vC_p2;

% solve delta_V total for the chase maneuver
for k = 1:length(MeC_p)
deltaV_total(k) = norm(deltaVb(:,k)) + norm(deltaVc_p(:,k));

% Orbital Paramenters
orb_Elements(k,:) = coe_from_sv(rB1,vB2(:,k));
%coe_from_sv gives [h e RA incl w TA a]
end

% Plots and Tables required
tInv = delta_timRang./60; % convert to minutes

figure; % Plot 1
plot(tInv,deltaV_total,'b-','DisplayName','Transfer Tradeoffs');
hold on; grid on;
ylabel('\DeltaV required [ km/s ]');
xlabel('intercept time [ mins ]');
title({'Tradeoffs Between Intercept Time and Total \DeltaV','Chase Maneuvers'});
set(gca,'FontSize',14);

% save plot to file
%saveas(gca,'task3plot1.jpg');
%
% Table 1
h = table(orb_Elements(1:7,2),orb_Elements(1:7,1),deg.*orb_Elements(1:7,6));
h.Properties.VariableNames = {'e' 'h' 'TA'};
disp(h)

% intercept trajectory
eint = orb_Elements(1,2); % use first one to match book example
hint = orb_Elements(1,1); % h
taint = deg*orb_Elements(1,6); % true anaomoly in it's own frame
pangint = (360 - taint) + 45; % get the perigee angle in geo frame

rint = @(theta) hint^2/(398600*(1+eint*cosd(theta)));
thetas = [-75:120];
for idx=1:length(thetas)
	r = rint(thetas(idx));
	ta = pangint + thetas(idx); % need to rotate by the perigee angle rotation
	yvals(idx) = r * sind(ta);
	xvals(idx) = r * cosd(ta);
end
...
figure;
hold on; grid on;
ellipse(6378,6378,0,0,0,'b')
ellipse(a_orb1,b_orb1,0,-foc1,0,'k')
plot(xvals,yvals,'r--');
scatter([rB1(1,1) rC1(1,1) rC_p(1,1)],[rB1(2,1) rC1(2,1) rC_p(2,1)],'r');
text(-.6e4,0,'Earth','FontSize',14);
text(-2.26e4,0,'Orbit 1','FontSize',14);
text(-1.5e4,0,'Intercept Trajectory','FontSize',14);
text(rB1(1,1)+.1e4,rB1(2,1),'B','FontSize',14);
text(rC1(1,1)-.13e4,rC1(2,1),'C','FontSize',14);
text(rC_p(1,1)-.14e4,rC_p(2,1),'C''','FontSize',14);
xlim([-2.5e4 1.5e4])
ylim([-1.5e4 1.5e4])
set(gca,'FontSize',14);
